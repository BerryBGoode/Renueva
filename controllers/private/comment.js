...
async function onSearch(evt) {
    ROWS.innerHTML = ``;
    evt.preventDefault();
    /* enviar datos de la url para seleccionar un producto especifico */
    // instancia de la clase FormData
    const DATA = new FormData;
    // adjuntar el id del product obteniendolo de la url
    DATA.append('idproduct', getProductURL());
    const JSON = await dataRequest(COMMENT, 'comments', DATA);
    if (!JSON.status) {
        notificationRedirect('error', JSON.exception, false);
        ...
        if (search === '') {
            ...
            for (let comments of JSON.dataset) {
                let username = comments.username.toLowerCase();
                // let product = comments.product.toLowerCase();
                // no tiene sentido buscar por el nombre del producto
                // porque ya solo acceder a está vista solo muestra 
                // los comentario de un producto
                if (username.indexOf(search) !== -1) {
                    ...
                    <td>${comments.date_comment}</td>
                    <td>

                        <!-- boton para eliminar -->
                        <svg width="22" height="25" viewBox="0 0 30 33" fill="none" onclick="onDestroy(${comments.id_review})"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M28.4432 7.7208C23.4903 7.23955 18.5076 6.99164 13.5398 6.99164C10.5948 6.99164 7.64985 7.13747 4.70487 7.42914L1.67065 7.7208"
                                stroke="#424242" stroke-width="3" stroke-linecap="round"
